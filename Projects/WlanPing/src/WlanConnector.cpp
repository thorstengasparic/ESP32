...
WlanConnector::WlanConnector(uint8_t accesPointPinIn, uint8_t statusLed, String version, bool WIFIOnly)
{
  webserver = NULL;
...
void   WlanConnector::ResetWebserver()
{
  if (webserver != NULL)
  {
    webserver->close();
...
void WlanConnector::PrepareWebserver()
{
  if (WIFIOnly && !AccessPointMode()) return;
...
void WlanConnector::ProcessClient()
{
  if (webserver != NULL){
...
{
  #ifdef DEBUG_WLAN
    Serial.println("handle_OnAccessPoint");
  #endif
...
{
  #ifdef DEBUG_WLAN
    Serial.println("handle_OnNotFound");
  #endif
  this->resetWDT();
