...
bool Adafruit_ADS1X15::DeviceDetected(void)
{
  return m_i2c_dev->detected();
}
/**************************************************************************/
/*!
...
bool Adafruit_ADS1X15::writeRegister(uint8_t reg, uint16_t value) {
  ...
  buffer[1] = value >> 8;
  buffer[2] = value & 0xFF;
  lastRequestOK =  m_i2c_dev->write(buffer, 3);
...
uint16_t Adafruit_ADS1X15::readRegister(uint8_t reg) {
  buffer[0] = reg;
  lastRequestOK = m_i2c_dev->write(buffer, 1);
  if (!lastRequestOK) return 0;
  lastRequestOK = m_i2c_dev->read(buffer, 2);
  if (!lastRequestOK) return 0;
